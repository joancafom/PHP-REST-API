    ...
    OpenIDAuthorizationCodeInterface
{
    ...
    public function getAccessToken($access_token)
    {
        ...
        if ($token = $stmt->fetch(\PDO::FETCH_ASSOC)) {
			...
			$fechaSinMilis = substr($token['EXPIRES'],0,-7);
            $fechaDateTime = \DateTime::createFromFormat("d/m/y G:i:s", $fechaSinMilis);
			
            $token['EXPIRES'] = $fechaDateTime->getTimestamp();
        }
    ...
    public function setAccessToken($access_token, $client_id, $user_id, $expires, $scope = null)
    {
		...
		//echo 'UPDATE '.$this->config['access_token_table']. ' SET client_id='.$client_id.', expires='.$expires.', user_id='.$user_id.', scope='.$scope.' where access_token='.$access_token.';';
    ...
    {
        $stmt = $this->db->prepare($sql = sprintf('SELECT * FROM %s WHERE issuer=:client_id AND subject=:subject AND audience=:audience AND expires=TO_DATE(:expires, \'YYYY-MM-DD HH24:MI:SS\') AND jti=:jti', $this->config['jti_table']));
